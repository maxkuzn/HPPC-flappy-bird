...
class np_mlp(object):
    def __init__(self, n_objects = 50, in_dim = 3, mid_dim = 7, out_dim = 1):
        self.W1 = np.random.uniform(-1,1, size = (n_objects, mid_dim, in_dim))*np.sqrt(in_dim+mid_dim)
        self.b1 = np.random.uniform(-1,1, size=(n_objects, mid_dim))*np.sqrt(in_dim+mid_dim)
        if out_dim == 1:
            self.W2 = np.random.uniform(-1,1, size=(n_objects, mid_dim))*np.sqrt(mid_dim+out_dim)
